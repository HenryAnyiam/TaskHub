...
def login():
    ...
    error = "None"
    if email:
        user = storage.search(User, User.email, email)
        if user:
...
def create_task():
    ...
    if title:
        """create the task from user input"""
        team = request.form.get('team', current_user.id)
        if team != user_id:
            team = all_team[f'Team.{team}'].id
        ...
        task.save()
        if parent != 'None':
            """add task to parents child task list"""
            parent = all_task.get(f'Task.{parent}')
            if parent:
        ...
        if team != user_id:
            """add tasks to team task list"""
            team_obj = all_team.get(f'Team.{team}')
            if team_obj:
                hold = team_obj.tasks
    ...
    for i in all_team:
        """get team user is part of"""
        members = all_team[i].get('members')
        if members:
            if user_id in members:
                teams.append(all_team[i])
    min_date = date.today().isoformat()
    parent_id = request.form.get('parent_id', "None")
    members = "None"
    if parent_id != "None":
        parent = all_task.get(f'Task.{parent_id}')
        if (parent is not None) and (parent.created_by != parent.team):
            members = all_team.get(f'Team.{parent.team}', "None")
        elif (parent is not None) and (parent.created_by == parent.team):
            all_user = storage.all('User')
            members = all_user.get(f'User.{parent.team}')
...
def view_tasks():
    ...
    for i in range(length):
        ...
        team_obj = all_team.get(f'Team.{team_id}', all_user.get(f'User.{team_id}'))
        if team_obj:
...
def view_teams():
    ...
    for i in range(length):
        ...
        for j in range(lent):
            ...
            all_user = storage.all('User')
            teams[i]['members'][j] = all_user.get(f'User.{userId}')
    for i in range(length):
        hold = teams[i]['tasks']
        ...
        for j in range(lent):
            ...
            all_task = storage.all('Task')
            teams[i]['tasks'][j] = all_task.get(f'Task.{taskId}')
...
def work_board():
    ...
    all_team = storage.all('Team')
    if new_message:
        team_detail = all_team.get(f'Team.{team}')
    ...
    if p_task:
        taskId = p_task
        p_task = all_task.get(f'Task.{p_task}')
        if p_task:
            p_task.progress = 100
            ...
            length = len(child)
            for i in range(length):
                child[i] = all_task.get(f'Task.{child[i]}')
    ...
    if c_task:
        c_task = all_task.get(f'Task.{c_task}')
        if c_task:
            c_task.progress = 100
            parent = all_task.get(f'Task.{c_task.parent}')
            if parent:
                taskId = parent.id
                hold = parent.child_task
            ...
            storage.save()
    if taskId:
        task = all_task.get(f'Task.{taskId}')
        if task:
            parent = task.parent
            parent_task = all_task.get(f'Task.{parent}')
            if parent_task:
            ...
            child_tasks = child if len(child) > 0 else 'None'
            all_user = storage.all('User')
            if (main_task.team != current_user.id) and (all_team.get(f'Team.{main_task.team}')):
                messages = []
                all_msg = storage.all('Message')
                for i in all_msg:
                    if main_task.team == all_msg[i].receiver:
                ...
                if length > 0:
                    ...
                    for i in range(length):
                        messages[i]['sender'] = all_user.get(f"User.{messages[i]['sender']}")
            if child_tasks != 'None':
                length = len(child_tasks)
                for i in range(length):
                    child_tasks[i] = child_tasks[i].to_dict()
                    child_tasks[i]['team'] = all_user.get(f"User.{child_tasks[i]['team']}", 
                                                     all_team.get(f"Team.{child_tasks[i]['team']}"))
            main_task = main_task.to_dict()
            main_task['team'] = all_user.get(f"User.{main_task['team']}", all_team.get(f"Team.{main_task['team']}"))
